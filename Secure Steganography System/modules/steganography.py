    ...
    def _parse_png(self, png_data: bytes):
        ...
        for y in range(h):
            ...
            if ft == 1:  # Sub
                ...
                if y > 0:
                    pr_s = (y - 1) * row_len + 1
                    pr = bytearray(raw[pr_s: pr_s + w * bpp])  # type: ignore[index]
                    for i in range(len(rd)):
                        rd[i] = (rd[i] + pr[i]) & 0xFF
            elif ft == 3:  # Average
                pr = bytearray(raw[(y - 1) * row_len + 1: (y - 1) * row_len + 1 + w * bpp]) if y > 0 else bytearray(w * bpp)  # type: ignore[index]
                for i in range(len(rd)):
